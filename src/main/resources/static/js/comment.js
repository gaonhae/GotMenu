...
function getCommentSuccess(data) {
	...
	for (var i = 0; i < data.array.length; i++) {
		...
		commentHtml +=
			...
			'				<a class="writerId" href="' +
			userDetail +
			'"> ' +
			data.array[i].writerId +
			"</a>" +
			"				<label>" +
			data.array[i].resistDate +
			"</label>" +
			"			</div>" +
			"			<div>" +
			'				<textarea class="commentContent" cols=50 rows=3 disabled>' +
			data.array[i].content +
			"</textarea>" +
			"			</div>";

		if (data.array[i].isSameUser)
			// 만약 현재 유저와 작성자가 같다면 수정/삭제 버튼을 보여준다.
			commentHtml += '			<div class="buttonBox">' + '				<button onclick="' + update + '">수정</button> <button onclick="' + remove + '">삭제</button>' + "			</div>" + "		</div>";
		// 아니라면 그냥 div를 닫아준다.
		else commentHtml += "		</div>";
	}
